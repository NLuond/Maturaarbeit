...
namespace cfg {
    ...
    constexpr int   ACCEL_ODR_HZ    = 1660;
    constexpr int   ACCEL_BW_HZ     = 400; 
    constexpr int   GYRO_RANGE_DPS  = 500;
    constexpr int   GYRO_ODR_HZ     = 416;
    ...
    constexpr float    PINCH_GYRO_GUARD = 100.f;  // über diesem gyroSum = Bewegung, kein Pinch
    ...
    constexpr uint32_t MOVE_INTERVAL_US = 8000;
    ...
    // ML-Classification
    constexpr float ML_CONFIDENCE = 0.7f;
    // Haptik (Vibrationsmotor)
    // Haptik (Vibrationsmotor, PWM)
    // Haptik (Vibrationsmotor, PWM)
    constexpr int      HAPTIC_PIN        = D1;
    constexpr uint8_t  HAPTIC_PEAK       = 150;   // dezente Variante
    constexpr uint32_t HAPTIC_ATTACK_MS  = 10;    // kurzer Anschlag
    constexpr uint32_t HAPTIC_MS         = 45;    // Gesamtdauer inkl. Ausklang
}